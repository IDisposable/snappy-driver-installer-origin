...
`internal/settings/flags.go`, `filters.go`, `settings.go`,
`compat.go`, `parse.go`, `persistence.go`, and `README.md` existed -
...
`docs/PORTING_NOTES.md` for their real (mostly Done) status. A few
...
Go (`matcher.cpp` listed as not started in `README.md`).
...
| Binary index files (`indexes/**/*.bin`) | Not discussed at the byte level in the manual at all (out of scope for a user manual) | Not done - `indexing.cpp` not started. The `"SDW"`+LZMA container format is separately reverse-engineered in `README.md`, not sourced from this manual. |
| State snapshots (`logs/*.snp`) | `-ls:<file>` loads one; `-nosnapshot` suppresses creation; script commands `snapshot [filename]` / `loadsnapshot <filename>` / `unloadsnapshot` manage them; default snapshot file name in script mode has no timestamp unless `-nostamp` is absent | Partial - `internal/scan.Prepare` writes a timestamped `logs/<ts>state.snp` after every scan (`internal/scan/snapshot.go`), honoring `-nosnapshot`, using the SDW container with a Go-native JSON payload (redesigned per README/COMPATIBILITY.md, not the original's raw struct dump). `-ls:<file>` (`StateModeEmul`) now replays a saved snapshot's System/Devices instead of scanning real hardware (`loadSnapshot`, wired into `Prepare`) - only Go-written snapshots decode (rejects a real C++ VER_STATE 0x102 file by design, see snapshotVersion's doc comment). Script mode's `snapshot`/`loadsnapshot`/`unloadsnapshot` commands are still not done (`script.cpp` not started) |
...
build tag or similar), not yet reflected in `README.md`'s module
