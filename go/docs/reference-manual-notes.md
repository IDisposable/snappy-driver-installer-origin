...
| State snapshots (`logs/*.snp`) | `-ls:<file>` loads one; `-nosnapshot` suppresses creation; script commands `snapshot [filename]` / `loadsnapshot <filename>` / `unloadsnapshot` manage them; default snapshot file name in script mode has no timestamp unless `-nostamp` is absent | Partial - `Settings.StateFile`/`StateMode` exist and are wired to `-ls:`; actual snapshot save/load (`model.cpp`'s `State::save/load`) is explicitly called out in README as free to redesign, not required to be byte-compatible |
