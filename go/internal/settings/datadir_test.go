...
// them) and leaves DrpDir/IndexDir/OutputDir/LogDirRaw at New()'s bare
// relative defaults - the same paths every other os.* call in this
// codebase already resolves against the working directory.
func TestResolveDataDirsPortableViaWorkingDirectory(t *testing.T) {
	...
	s := New()
	wantDrpDir, wantIndexDir, wantOutputDir, wantLogDir := s.DrpDir, s.IndexDir, s.OutputDir, s.LogDirRaw
	...
	if s.DrpDir != wantDrpDir || s.IndexDir != wantIndexDir || s.OutputDir != wantOutputDir || s.LogDirRaw != wantLogDir {
		t.Errorf("ResolveDataDirs() changed directories for a portable working directory: DrpDir=%q IndexDir=%q OutputDir=%q LogDirRaw=%q",
			s.DrpDir, s.IndexDir, s.OutputDir, s.LogDirRaw)
	}
}
...
// elsewhere), ResolveDataDirs redirects DrpDir/IndexDir under the
// user's cache directory (%LOCALAPPDATA% on Windows) instead of
// leaving them at the bare "drivers"/"indexes" that would otherwise
