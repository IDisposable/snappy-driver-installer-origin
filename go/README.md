...
| `indexing.cpp`: `Driverpack::scaninf` (matching an installed driver back to the .inf that produced it, recovering its feature score/catalog bits) | - | Not started | Needs the not-yet-ported `genindex`/inf-cache write-side orchestration. Blocks `calc_status`'s installed-driver comparison and `enum.cpp`'s `Driver::scaninf` caller. |
