...
| `indexing.cpp`/`matcher.cpp`: `Collection` (folder scanning + `Driverpack` lifecycle), `manager.cpp` | - | Not started | `internal/indexing.ScanDriverpackFolder` + `sdwfile`/`DecodeIndex` already cover loading one pack's index on demand; a `Collection` wrapper to load/cache all of them, plus `Manager`'s command/state orchestration, isn't built yet. |
