...
| `indexing.cpp`: `.cat` catalog file parsing, `genindex` threading/orchestration, `Txt` interning to build new `.bin` files (`Driverpack::saveindex`'s write path) | - | Not started | The read side (`DecodeIndex`) is done; this is the write side, to build/update a collection from scratch. |
...
| `matcher.cpp`: full match orchestration (`Hwidmatch`/`Devicematch`/`Matcher`, `calc_status`, `calc_altsectscore`, vendor-specific validity checks) | - | Not started | Needs the not-yet-ported `Driverpack`/`Collection` object graph and several vendor-specific heuristics (Intel USB3 hub validation, Realtek blacklist, etc.) read but not yet ported. |
