...
`cmd/sdi` | `cmd/sdi` | Plain-text report + gated install | A working CLI entry point tying the whole engine together: scan hardware, load a driver-pack collection, match every device, print a report. Not the planned TUI yet (see below). **Milestone: verified end-to-end against a real machine and its real driver-pack collection** (371 devices, 121 packs, 0 skipped) via WSL PE interop, now with real BETTER/WORSE/SAME-vs-installed comparison (see `Driver::scaninf` below) - 32 devices genuinely benefit from a driver-pack candidate, the rest correctly reported as already having their best driver (an earlier pre-`scaninf` version of this milestone reported 93 "found," which testing against real data proved included 61 false positives - see below). This is the first time every ported piece (hardware, indexing, matcher, collection) has run together against real data in one program. A `-install` flag (off by default; scan-and-report requires no flag) wires up the full extract-then-install path: `archive.ExtractPrefix` (new - pulls a driver's whole folder, not just its `.inf`, out of the `.7z`) into `Settings.ExtractDir`, then `internal/install.Driver`, then `internal/install.RemoveExtraInfs` if `-delextrainfs` is set. Before any installs, it does the full restore-point-frequency-throttle-bypass sequence (`Get`/`SetRestorePointCreationFrequency(0)`/`CreateRestorePoint`/`SetRestorePointCreationFrequency(original)`) unless `-disableinstall` is set - an earlier version only called `CreateRestorePoint` directly, which Windows' one-per-24h default throttle could silently no-op. `Settings.Save(DefaultCfgFilename)` runs on exit (even if the run errored), so command-line switches persist into `sdio.cfg` like the original's unconditional `Settings.save()` - verified round-tripping a real `sdio.cfg` (legacy syntax in, new syntax out, GUI-only fields correctly dropped) in an isolated scratch copy, never against the real reference installation's file. The install/extract path itself is verified by construction and by `internal/archive`'s `ExtractPrefix` tests against a real archive, but - unlike the scan/report path and the torrent download - was deliberately never invoked live against the real machine, since it modifies real driver state. |
