...
| `matcher.cpp`: `Hwidmatch`/`Devicematch`/`Matcher` object construction, `cmpnames` | - | Not started | Needs the not-yet-ported `Driverpack`/`Collection`/`Device` object graph (torrent-download bookkeeping, `Collection::scanfolder` wiring `Driverpack` instances together) and `Manager`. |
