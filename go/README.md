...
`cmd/sdi` | `cmd/sdi` | Plain-text report | A working CLI entry point tying the whole engine together: scan hardware, load a driver-pack collection, match every device, print a report. Not the planned TUI yet (see below) and doesn't install anything - a scan-and-report MVP. **Milestone: verified end-to-end against a real machine and its real driver-pack collection** (371 devices, 121 packs, 0 skipped) via WSL PE interop - 93 devices found a better/alternate driver with plausible vendor/section/version metadata (e.g. a real AMD Radeon 780M matched to the correct `DP_Videos_AMD_DCH32` pack), the rest correctly reported as already having their best driver. This is the first time every ported piece (hardware, indexing, matcher, collection) has run together against real data in one program. |
