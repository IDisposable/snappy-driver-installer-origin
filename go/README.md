...
| `indexing.cpp`: `.cat` catalog file parsing, `genindex` threading/orchestration, `Txt` interning to build new `.bin` files (`Driverpack::saveindex`'s write path) | - | Not started | `Find`/`FindNext` (chained hash lookup) also not yet ported - needed by `matcher.cpp`. |
| `matcher.cpp`: hardware-to-driver matching/ranking (Missing/Newer/Current/Older/Better/Worse, `calc_decorscore`, `calc_markerscore`, `genmarker`) | - | Not started | Depends on the indexing work above. |
